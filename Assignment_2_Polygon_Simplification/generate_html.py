...
def implementation_cards():
    return (
        "<div class='card-grid'>"
        "<div class='info-card'><h4>Core Representation</h4><p><strong>Rings:</strong> Each ring is stored as a circular doubly linked list, so removing a vertex only changes a constant-size neighborhood.</p><p><strong>Why it matters:</strong> This directly supports the repeated local updates required by APSC.</p></div>"
        "<div class='info-card'><h4>Candidate Selection</h4><p><strong>Priority queue:</strong> Collapse candidates are ranked by estimated areal displacement.</p><p><strong>Why it matters:</strong> The algorithm avoids expensive global rescans after every accepted collapse.</p></div>"
        "<div class='info-card'><h4>Topology Checks</h4><p><strong>Spatial index:</strong> A lightweight uniform grid accelerates intersection queries against nearby edges.</p><p><strong>Why it matters:</strong> This keeps topology preservation practical even on large inputs.</p></div>"
        "<div class='info-card'><h4>Local Updates</h4><p><strong>Incremental maintenance:</strong> Only candidates near the modified neighborhood are refreshed after a collapse.</p><p><strong>Why it matters:</strong> This is the main reason the measured runtime scales much better than naive full recomputation.</p></div>"
        "<div class='info-card'><h4>Enhancement 1</h4><p><strong>Candidate bias:</strong> Interpolated fallback placements are penalized more than boundary-intersection candidates.</p><p><strong>Effect:</strong> This reduced total areal displacement on difficult reference cases such as <code>cushion_with_hexagonal_hole</code> and <code>original_08</code>.</p></div>"
        "<div class='info-card'><h4>Enhancement 2</h4><p><strong>Safety guards:</strong> Stale queue entries are rejected unless they still describe a valid consecutive <code>A-B-C-D</code> neighborhood, and the exterior ring is not collapsed below four vertices.</p><p><strong>Effect:</strong> This prevents invalid late-stage moves and keeps multi-ring cases such as <code>rectangle_with_two_holes</code> stable.</p></div>"
        "</div>"
    )
...
def main():
    ...
    html += """<header><h1>Assignment 2 - Results Report</h1><nav><a href='#instructions'>Instructions</a><a href='#algorithm'>Algorithm</a><a href='#dataset-goals'>Dataset Goals</a><a href='#overview'>Overview</a><a href='#efficiency'>Efficiency</a><a href='#displacement'>Displacement</a><a href='#custom-cases'>Custom Cases</a><a href='#discussion'>Discussion</a><a href='#correctness'>Visuals</a></nav></header><div class='container'>"""
    html += "<section id='instructions'><h2>Instructions</h2><div class='instructions-box'><p>This report is organized to satisfy the experimental-evaluation requirements of the assignment.</p><ul><li><strong>Algorithm</strong> explains the main data structures and the implementation improvements beyond baseline APSC.</li><li><strong>Dataset Goals</strong> explains what each dataset targets and why it is difficult.</li><li><strong>Overview</strong> keeps the exact values from the output files for area, displacement, running time, and peak memory.</li><li><strong>Efficiency</strong> shows running time vs input size and peak memory vs input size, together with fitted scaling laws.</li><li><strong>Displacement</strong> compares displacement quality, final vertex counts, displacement vs input size, and displacement vs target vertex count.</li><li><strong>Custom Cases</strong> explains the purpose of the custom stress tests.</li><li><strong>Discussion</strong> interprets the main trends rather than only listing raw values.</li></ul></div></section>"
    html += f"<section id='algorithm'><h2>Algorithm Design and Enhancements</h2><p class='placeholder-text'>This section addresses the rubric items on data structures, local updates, and evidence of improvements beyond a bare-bones APSC implementation.</p>{implementation_cards()}</section>"
